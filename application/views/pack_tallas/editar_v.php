...
<head>
    ...
    <body class=" sidebar_main_open sidebar_main_swipe">
        ...
        <form action="" class="uk-form-stacked" method="POST"  name="formulario_modificar" id="formulario_modificar">
            <?php foreach($pack_tallas as $values){ ?>
            <h3 class="heading_b uk-margin-bottom">MODIFICAR PACK TALLAS</h3>
                        ...
                        <div class="uk-grid uk-grid-divider uk-grid-medium" data-uk-grid-margin>
                            <div class="uk-width-large-1-1">
                                <div class="uk-form-row">
                                    ...
                                    <input type="text" class="md-input" autocomplete="off" style="text-transform: uppercase;" id="pack_tallas"   name="pack_tallas" value="<?php echo $values->pack_tallas; ?>" />
                                    <input type="hidden" id="id_pack_tallas" name="id_pack_tallas"  value="<?php echo $values->id_pack_tallas; ?>" />
                                </div>
                                <div class="uk-form-row">
                                    ...
                                    <select id="kUI_multiselect_basic" name="tallas[]" multiple="multiple" data-placeholder="SELECCIONA LOS COLORES">
                                        ...
                                        $vali='0';
                                        foreach($detalle_tallas as $valuen){  
                                          if($value->id_tallas==$valuen->id_tallas){
                                            $vali='1';
                                          }
                                        }
                                        if($vali=='0'){
                                       ...
                                       <?php }else{ ?>
                                         <option value="<?php echo $value->id_tallas; ?>"  selected="selected" ><?php echo $value->descripcion; ?></option>
                                       <?php } } ?>
                                    </select>
                                </div>
                            </div>
        ...
        <script> 
          ...
          $('#formulario_modificar').on("submit", function(e){
            ...
            if (pack_tallas!=''&&tallas!=null) {
              ...
              var url = "<?php echo base_url();?>Pack_tallas_c/modificar";
              $.ajax({                        
                 type: "POST",                 
                 url: url,                     
                 data: formData,
            ...
            swal({
                title: "Error al Registrar Pack Tallas",
                text: "¡Revise los campos del Pack Tallas!",
                type: "error",
